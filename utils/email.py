...
def generate_html_change_product(product_change: ProductChange) -> str:
    ...
    for key, value in product_change.product_change.items():
        html_change = f"""<tr style="border: 1px solid black; border-collapse: collapse;"> 
            <td style="border: 1px solid black; border-collapse: collapse;font-weight: bold;">{key}</td> 
            <td sytle="border: 1px solid black; border-collapse: collapse;">{value}</td> 
        </tr>"""
    ...
    return f"""
        <html>
            ...
            <p>The product with id "{product_change.product_id}" and sku "{product_change.sku}"
